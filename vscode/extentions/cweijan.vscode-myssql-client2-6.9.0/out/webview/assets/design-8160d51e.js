...
BEGIN 
    $2
    RETURN x*2;
END;`}}class te extends Y{showVersion(){return`SELECT REPLACE(banner,'DM Database Server 64 ','') "server_version"  FROM v$version where rownum=1;`}createIndex(e){const{table:t,column:n="$2"}=e;return`CREATE INDEX ${`${t}_${n}`} ON ${t}(${n});`}dropIndex(e,t){return`DROP INDEX ${t};`}showIndex(e,t){return`SELECT COLUMN_NAME "column_name",INDEX_NAME "index_name" from DBA_IND_COLUMNS WHERE TABLE_OWNER='${e}' and TABLE_NAME='${t}';`}variableList(){return"show global VARIABLES"}statusList(){return"show global status"}processList(){return"show processlist"}addColumn(e,t){return`ALTER TABLE ${e} 
    ADD COLUMN $1 $2;
...
COMMENT ON COLUMN ${e}.${n} IS '${s||"comment"}';`}updateColumnSql(e){return new G(e).genAlterSQL()}showUsers(){return'SELECT usename "user" from pg_user '}showForeignKeys(e,t){return`SELECT
...
BEGIN
    ...
    return 0;
END;
$$ LANGUAGE plpgsql;`}}class oe extends V{showVersion(){return"SELECT VERSION() as server_version"}createIndex(e){throw new Error("Redshift not support index!")}variableList(){return"SHOW ALL"}statusList(){return`SELECT
        'db_numbackends' AS db,
        pg_stat_get_db_numbackends(datid) AS status
      FROM
        pg_stat_database
...
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")`}updateColumn(e,t){var N;const{name:n,type:r,comment:s,nullable:o,defaultValue:l,extra:i,character_set_name:E,collation_name:T}=t,d=o!="YES";return new m(`ALTER TABLE ${e}`).append(`
	CHANGE ${n} ${n} ${r}`).if(E,`CHARACTER SET ${E}`).if(T,`COLLATE ${T}`).if(d,"NOT NULL").if((N=i==null?void 0:i.toLowerCase())==null?void 0:N.includes("auto_increment"),"AUTO_INCREMENT").if(s,`COMMENT '${s}'`).if(_(l)&&!d,"DEFAULT NULL").if(!_(l),`DEFAULT ${l=="CURRENT_TIMESTAMP"?l:`'${z(l)}'`}`).toString()}updateColumnSql(e){const{table:t,columnName:n,newColumnName:r,columnType:s,isNotNull:o,isAutoIncrement:l,comment:i,defaultValue:E,oldRow:T}=e,d=`ALTER TABLE "${t}"`,N=o?"SET NOT NULL":"DROP NOT NULL";return new m(`${d} ALTER COLUMN "${n}" TYPE ${s};`,`
...
ALTER TABLE ${e} 
    RENAME COLUMN ${n} TO ${n};`}updateColumnSql(e){return new v(e).genAlterSQL()}}class ce extends I{showVersion(){return"select version() as server_version;"}showDatabases(){return"show databases"}showTables(e,t){return`show tables in ${e}`}showColumns(e,t){return`describe ${e}.${t};`}showViews(e,t){return`show views in ${e}`}}class Ee extends I{showVersion(){return"SELECT node_version as server_version FROM system.runtime.nodes;"}pingDataBase(e){return e?`use ${e}`:"select 1"}showDatabases(){return"show catalogs"}}class ue extends I{showVersion(){return"SELECT cql_version as server_version FROM system.local;"}pingDataBase(e){return e?`use ${e}`:"select 1"}showDatabases(){return'select keyspace_name as "database" from system_schema.keyspaces;'}showTables(e,t){return`select table_name as "name" from system_schema.tables where keyspace_name='${e}'`}showColumns(e,t){return`select column_name as "name",type from system_schema.columns where keyspace_name='${e}' and table_name='${t}'`}}class me extends v{otherPart(){const{oldRow:e,isAutoIncrement:t}=this.param;return!e.isAutoIncrement&&t?`${this.afterColumnPrefix} SET GENERATED ALWAYS as identity;`:e.isAutoIncrement&&!t?`${this.afterColumnPrefix} DROP GENERATED;`:null}commentPart(){const{oldRow:e,table:t,columnName:n,comment:r}=this.param;if(r!=e.comment)return`COMMENT ON COLUMN "${t}"."${n}" is '${r}';`}changeTypePart(){const{oldRow:e,columnType:t}=this.param;return e.type==t?"":`${this.afterColumnPrefix} SET DATA TYPE ${t};`}}class de extends I{showVersion(){return'SELECT SERVICE_LEVEL as "server_version" FROM SYSIBMADM.ENV_INST_INFO;'}pingDataBase(e){return e?`set SCHEMA ${e}`:"select 1"}showDatabases(){return'select schemaname "database" from syscat.schemata'}showTableSource(e,t){return null}showTables(e,t){return`select tabname "name", remarks "comment" from syscat.tables where tabschema='${t}' and TYPE='T' order by tabname`}showColumns(e,t){return`select COLUMN_NAME "name",
        ...
        UNIQUERULE='U' "isUnique"
         FROM SYSCAT.INDEXES WHERE TABNAME = '${t}' AND TABSCHEMA = '${e}'`}showViews(e,t){return`select VIEWNAME "name",TEXT "source" from SYSCAT.VIEWS where VIEWSCHEMA = '${t}';`}showProcedures(e){return`select PROCNAME "routine_name",TEXT "source" from SYSCAT.procedures where PROCSCHEMA = '${e}' order by PROCNAME;`}showChecks(e,t){return`SELECT CONSTNAME "name",TEXT "clause" FROM SYSCAT.CHECKS WHERE TABNAME = '${t}' AND TABSCHEMA = '${e}' LIMIT 100;`}showFunctions(e){return`select FUNCNAME "routine_name",BODY "source" from SYSCAT.FUNCTIONS where FUNCSCHEMA = '${e}' order by FUNCNAME;`}dropIndex(e,t){return`DROP INDEX "${t}"`}tableTemplate(){return`CREATE TABLE table_name$1(  
    id int not NULL PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    create_time DATE,
    update_time DATE,
    content VARCHAR(255)
);`}}function Te(a){switch(a||(a=c.MYSQL),a){case null:case c.MYSQL:return new W;case c.MARIA_DB:return new ne;case c.MSSQL:return new re;case c.SQLITE:return new q;case c.DUCK_DB:return new ie;case c.REDSHIFT:return new oe;case c.SNOWFLAKE:return new le;case c.PG:return new V;case c.CLICKHOUSE:return new ee;case c.DM:return new te;case c.ORACLE:return new Y;case c.CASSANDRA:return new ue;case c.PRESTO:case c.TRINO:return new Ee;case c.HIVE:return new ce;case c.DB2:return new de;case c.MONGO_DB:return new ae;case c.NEO4J:return new se}return new I}class L{constructor(e){this.meta=e,this.dialect=Te(e.dbType),this.wrappedTable=S(this.meta.table,this.meta.dbType,J.Always)}updateTable(e){return this.dialect.updateTable(e)}columnTypes(){return["INTEGER",{label:"VARCHAR",value:"VARCHAR(255)"},{label:"CHAR",value:"CHAR(50)"},"hr","DATE","TIMESTAMP","hr","BOOLEAN","FLOAT","DOUBLE","hr","TEXT","JSON","BLOB"]}strategys(){return["RESTRICT","CASCADE","SET NULL","SET DEFAULT","NO ACTION"]}newColumn(e){const{name:t,type:n,isNotNull:r}=e,s=r?" NOT NULL":"";return`ALTER TABLE ${this.wrappedTable} ADD COLUMN ${S(t,this.meta.dbType)} ${n}${s}`}updateColumn(e){return this.dialect.updateColumnSql({...e,table:this.meta.table,schema:this.meta.schema})}dropColumn(e){return`ALTER TABLE ${this.wrappedTable} DROP COLUMN ${S(e,this.meta.dbType)}`}formatIndexType(e){return e.index_type}indexTypes(){return[{label:"INDEX",value:"INDEX"},{label:"UNIQUE",value:"UNIQUE"}]}dropIndex(e){return this.dialect.dropIndex(this.wrappedTable,e)}showChecks(){return this.dialect.showChecks(this.meta.schema,this.meta.table)}showForeignKeys(){return this.dialect.showForeignKeys(this.meta.schema,this.meta.table)}newForeignKey(e){const{column:t,foreignKeyName:n,refTable:r,refColumn:s,onUpdate:o,onDelete:l}=e,i=o=="NO ACTION"?"":` ON UPDATE ${o}`,E=l=="NO ACTION"?"":` ON DELETE ${l}`;return new m(`ALTER TABLE "${this.meta.table}"`).if(n,()=>`ADD CONSTRAINT ${n}`).if(!n,"ADD").append(`FOREIGN KEY ("${t}") REFERENCES "${r}"("${s}")${i}${E};`).toString()}dropForignKey(e){return`ALTER TABLE "${this.meta.table}" DROP CONSTRAINT ${e};`}newCheck(e){return`ALTER TABLE ${this.wrappedTable} ADD CHECK (${e});`}dropCheck(e){return`ALTER TABLE ${this.wrappedTable} DROP CONSTRAINT ${e};`}splitColumn(e,t='"'){return e.map(n=>t+n+t).join(",")}}class _e extends L{columnTypes(){return["Int32","String","Float32","hr","Date","DateTime","hr","BOOLEAN","UUID",{label:"Array",value:"Array(String)"},"hr","JSON",{label:"Tuple",value:"Tuple(String)"},{label:"Map",value:"Map(String,String)"}]}newColumn(e){const{name:t,type:n,defaultValue:r,comment:s}=e;return new m(`ALTER TABLE ${this.wrappedTable} ADD COLUMN "${t}" ${n}`).if(!_(r),`DEFAULT ${p(r,n)}`).if(s,`COMMENT '${s}'`).toString()}newIndex(e){const{columns:t}=e;return`ALTER TABLE "${this.meta.table}" ADD INDEX ${t.join("")}_${new Date().getTime()}_index expression TYPE type GRANULARITY value AFTER ${this.splitColumn(t)}`}showForeignKeys(){return null}showChecks(){return null}}class j extends L{newColumn(e){const{name:t,type:n,defaultValue:r,isNotNull:s}=e;return new m(`ALTER TABLE "${this.meta.schema}".${S(this.meta.table,this.meta.dbType)} ADD ${S(t,this.meta.dbType)} ${n}`).if(s,"NOT NULL").if(!_(r),`DEFAULT ${p(r,n)}`).toString()}dropColumn(e){return`ALTER TABLE "${this.meta.schema}".${this.wrappedTable} DROP COLUMN ${S(e,this.meta.dbType)}`}newIndex(e){const{table:t=this.meta.table,columns:n,type:r}=e,s=`${t}_${n.join("_")}_${new Date().getTime()}`;return`CREATE ${r=="UNIQUE"?"UNIQUE":""} INDEX ${s} ON "${this.meta.schema}"."${t}"(${this.splitColumn(n)});`}newCheck(e){return`ALTER TABLE "${this.meta.schema}".${this.wrappedTable} ADD CHECK (${e});`}dropCheck(e){return`ALTER TABLE "${this.meta.schema}".${this.wrappedTable} DROP CONSTRAINT ${e};`}dropForignKey(e){return`ALTER TABLE "${this.meta.schema}"."${this.meta.table}" DROP CONSTRAINT ${e};`}newForeignKey(e){return super.newForeignKey(e).replace(`"${this.meta.table}"`,`"${this.meta.schema}"."${this.meta.table}"`)}}class Ae extends L{indexTypes(){return[{label:"INDEX",value:"INDEX"},{label:"UNIQUE",value:"UNIQUE"},{label:"FULLTEXT",value:"FULLTEXT"}]}columnTypes(){return["INT",{label:"VARCHAR",value:"VARCHAR(255)"},{label:"CHAR",value:"CHAR(50)"},"hr","DATETIME","TIMESTAMP","DATE","hr","BIT","FLOAT","DOUBLE",{label:"DECIMAL",value:"DECIMAL(10,2)"},"BIGINT","hr","TEXT","JSON","BLOB","BINARY","hr",{label:"ENUM",value:"ENUM('item')"},{label:"SET",value:"SET('item')"}]}newColumn(e){const{name:t,type:n,comment:r,defaultValue:s,isNotNull:o,unsigned:l,zerofill:i}=e;return console.log(e),new m(`ALTER TABLE ${this.wrappedTable} ADD COLUMN \`${t}\` ${n}`).if(l&&l!=0,"UNSIGNED").if(i,"ZEROFILL").if(o,"NOT NULL").if(r,`COMMENT '${r}'`).if(!_(s),`DEFAULT ${p(s,n)}`).toString()}formatIndexType(e){const t=e.index_type.toUpperCase();return t=="BTREE"?e.index_name=="PRIMARY"?"PRIMARY":P(e.isUnique)?"UNIQUE":"KEY":t}newIndex(e){const{type:t="key",columns:n}=e;return`ALTER TABLE \`${this.meta.table}\` ADD ${t} (${this.splitColumn(n,"`")})`}newForeignKey(e){const{column:t,foreignKeyName:n,refTable:r,refColumn:s,onUpdate:o,onDelete:l}=e,i=o=="NO ACTION"?"":` ON UPDATE ${o}`,E=l=="NO ACTION"?"":` ON DELETE ${l}`;return new m(`ALTER TABLE \`${this.meta.table}\``).if(n,()=>`ADD CONSTRAINT ${n}`).if(!n,"ADD").append(`FOREIGN KEY (\`${t}\`) REFERENCES \`${r}\`(\`${s}\`)${i}${E};`).toString()}dropForignKey(e){return`ALTER TABLE \`${this.meta.table}\` DROP FOREIGN KEY ${e};`}}class Ne extends L{newIndex(e){const{table:t=this.meta.table,columns:n,type:r}=e,s=`${t}_${n.join("_")}_${new Date().getTime()}`;return`CREATE ${r=="UNIQUE"?"UNIQUE":""} INDEX ${s} ON "${t}"(${this.splitColumn(n)})`}newColumn(e){const{name:t,type:n,isNotNull:r,defaultValue:s,comment:o}=e;return new m(`ALTER TABLE ${S(this.meta.table,this.meta.dbType)} ADD ${t} ${n}`).if(!_(s),`DEFAULT ${p(s,n)}`).if(r,"NOT NULL").if(o,`;
COMMENT ON COLUMN ${this.wrappedTable}."${t.toUpperCase()}" is '${o}';`).toString()}newForeignKey(e){const{column:t,foreignKeyName:n,refTable:r,refColumn:s,onUpdate:o,onDelete:l}=e,i=o=="NO ACTION"?"":` ON UPDATE ${o}`,E=l=="NO ACTION"?"":` ON DELETE ${l}`,T=n||`${t}_${r}_${s}_fk_${Math.floor(Math.random()*30)}`;return`ALTER TABLE "${this.meta.table}" ADD CONSTRAINT ${T} FOREIGN KEY ("${t}") REFERENCES ${r}(${s})${i}${E};`}showForeignKeys(){var t;let{schema:e}=this.meta;return(e==null?void 0:e.toLowerCase())==((t=this.meta.user)==null?void 0:t.toLowerCase())&&(e=void 0),this.dialect.showForeignKeys(e,this.meta.table)}}class pe extends L{newColumn(e){const{name:t,type:n,comment:r,defaultValue:s,isNotNull:o}=e,l=this.wrappedTable;return new m(`ALTER TABLE ${l} ADD COLUMN "${t}" ${n}`).if(o,"NOT NULL").if(!_(s),`DEFAULT ${p(s,n)}`).append(";").if(r,`
COMMENT ON COLUMN ${l}."${t}" is '${r}';`).toString()}columnTypes(){return["INTEGER",{label:"VARCHAR",value:"VARCHAR(255)"},{label:"CHAR",value:"CHAR(50)"},"TIMESTAMP","DATE","BOOLEAN","FLOAT","TEXT","JSON","BLOB"]}indexTypes(){return[{label:"INDEX",value:"INDEX"},{label:"UNIQUE",value:"UNIQUE"},{label:"HASH",value:"HASH"}]}newIndex(e){const{table:t=this.meta.table,columns:n,type:r}=e,s=r=="HASH"?"hash":"btree";return`CREATE ${r=="UNIQUE"?"UNIQUE":""} INDEX ${n.join("_")}_${new Date().getTime()}_index ON "${t}" USING ${s} (${this.splitColumn(n)});`}}class Re extends L{newIndex(e){throw new Error("Method not implemented.")}dropForignKey(e){return`ALTER TABLE "${this.meta.table}" DROP CONSTRAINT "${e}";`}showForeignKeys(){return`select
