...
BEGIN 
    $2
    RETURN x*2;
END;`}}class J extends k{showVersion(){return`SELECT REPLACE(banner,'DM Database Server 64 ','') "server_version"  FROM v$version where rownum=1;`}createIndex(e){const{table:t,column:n="$2"}=e;return`CREATE INDEX ${`${t}_${n}`} ON ${t}(${n});`}dropIndex(e,t){return`DROP INDEX ${t};`}showIndex(e,t){return`SELECT COLUMN_NAME "column_name",INDEX_NAME "index_name" from DBA_IND_COLUMNS WHERE TABLE_OWNER='${e}' and TABLE_NAME='${t}';`}variableList(){return"show global VARIABLES"}statusList(){return"show global status"}processList(){return"show processlist"}addColumn(e,t){return`ALTER TABLE ${e} 
    ADD COLUMN $1 $2;
...
}`}viewTemplate(){return""}procedureTemplate(){return""}triggerTemplate(){return""}functionTemplate(){return""}}class G extends N{showVersion(){return"select @@version server_version;"}createIndex(e){let t=`${e.type||"key"}`;return t.match(/BTREE/i)&&(t="key"),`ALTER TABLE ${e.table} ADD ${t} (\`${e.column||"$1"}\`)`}dropIndex(e,t){return`ALTER TABLE ${e} DROP INDEX \`${t}\``}showIndex(e,t){return`SELECT column_name "column_name",index_name "index_name",index_type "index_type",non_unique=0 "isUnique" FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema='${e}' and table_name='${t}';`}variableList(){return"show global VARIABLES"}statusList(){return"show global status"}processList(){return"show processlist"}addColumn(e,t){const n=t?` AFTER \`${t}\``:"";return`ALTER TABLE ${e} 
...
GRANT ALL PRIVILEGES ON *.* TO '$1'@'%' WITH GRANT OPTION;`}updateUser(e){return`update mysql.user set 
    ...
    where User = '${e}';
FLUSH PRIVILEGES;
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")`}updateColumn(e,t){var y;const{name:n,type:r,comment:o,nullable:s,defaultValue:l,extra:i,character_set_name:c,collation_name:m,unsigned:u,zerofill:_}=t,O=s!="YES",R=B(r);return new d(`ALTER TABLE ${e}`).append(`
	CHANGE ${n} ${n} ${r}`).if(u=="1","UNSIGNED").if(_=="1","ZEROFILL").if(!R&&c,`CHARACTER SET ${c}`).if(!R&&m,`COLLATE ${m}`).if(O,"NOT NULL").if((y=i==null?void 0:i.toLowerCase())==null?void 0:y.includes("auto_increment"),"AUTO_INCREMENT").if(o,`COMMENT '${o}'`).if(A(l)&&!O,"DEFAULT NULL").if(!A(l),`DEFAULT ${l=="CURRENT_TIMESTAMP"?l:`${L(l,r)}`}`).toString()}updateColumnSql(e){const{table:t,columnName:n,name:r,type:o,unsigned:s,zerofill:l,isNotNull:i,isAutoIncrement:c,comment:m,defaultValue:u,character_set_name:_,collation_name:O}=e,R=B(o);return new d(`ALTER TABLE \`${t}\``).append(`
	CHANGE \`${n}\` \`${r}\` ${o}`).if(s=="1","UNSIGNED").if(l=="1","ZEROFILL").if(!R&&_,`CHARACTER SET ${_}`).if(!R&&O,`COLLATE ${O}`).if(i,"NOT NULL").if(c,"AUTO_INCREMENT").if(m,`COMMENT '${m}'`).if(A(u)&&!i,"DEFAULT NULL").if(!A(u),`DEFAULT ${u=="CURRENT_TIMESTAMP"?u:`${L(u,o)}`}`).append(";").toString()}showCollations(){return'select DEFAULT_COLLATE_NAME "name",CHARACTER_SET_NAME "charset",DESCRIPTION "description" from information_schema.CHARACTER_SETS ORDER BY name;'}showChecks(e,t){return`SELECT
...
COMMENT ON COLUMN ${e}.${n} IS '${o||"comment"}';`}updateColumnSql(e){const{table:t,columnName:n,newColumnName:r,columnType:o,isNotNull:s,isAutoIncrement:l,comment:i,defaultValue:c,oldRow:m}=e,u=c==null?void 0:c.match(/nextval/),_=l&&!m.isAutoIncrement&&!u,O=!l&&m.isAutoIncrement&&!u,R=!l&&m.isAutoIncrement&&u,y=s?"SET NOT NULL":"DROP NOT NULL";return new d(`ALTER TABLE "${t}" ALTER COLUMN "${n}" TYPE ${o};`,`
`).append(`ALTER TABLE "${t}" ALTER COLUMN "${n}" ${y};`).if(_,`ALTER TABLE "${t}" ALTER "${n}" ADD GENERATED ALWAYS AS IDENTITY;`).if(O,`ALTER TABLE "${t}" ALTER "${n}" DROP IDENTITY;`).if(R,`ALTER TABLE "${t}" ALTER COLUMN "${n}" DROP DEFAULT;`).if(!R&&c!=null,`ALTER TABLE "${t}" ALTER COLUMN "${n}" SET DEFAULT ${c!=null&&c.match(/(:|nextval)/i)?c:`'${c}'`};`).if(i&&i!=m.comment,`COMMENT ON COLUMN "${t}"."${n}" is '${i}';`).if(n!=r,`ALTER TABLE "${t}" RENAME COLUMN "${n}" TO "${r}";`).toString()}showUsers(){return'SELECT usename "user" from pg_user '}pingDataBase(e){return e?`set schema '${e}';`:"select 1"}updateTable(e){const{table:t,newTableName:n,comment:r,newComment:o}=e;let s="";return o&&o!=r&&(s=`COMMENT ON TABLE "${t}" IS '${o}';`),n&&t!=n&&(s+=`ALTER TABLE "${t}" RENAME TO "${n}";`),s}truncateDatabase(e){return`SELECT Concat('TRUNCATE TABLE "',TABLE_NAME, '";') trun FROM INFORMATION_SCHEMA.TABLES WHERE  table_schema ='${e}' AND table_type='BASE TABLE';`}createDatabase(e){return"CREATE DATABASE $1"}showTableSource(e,t){return""}showViewSource(e,t){return`SELECT CONCAT('CREATE VIEW ',table_name,'
AS
...
WHERE schemaname='${e}') v order by v."name" ASC`}buildPageSql(e,t,n){return`SELECT * FROM ${t} LIMIT ${n};`}countSql(e,t){return`SELECT count(*) FROM ${t};`}showTables(e){return`SELECT t.table_name "name", pg_catalog.obj_description(pgc.oid, 'pg_class') "comment",pgc.reltuples "table_rows",pg_relation_size(pgc.oid) data_length
...
BEGIN
    ...
    return 0;
END;
$$ LANGUAGE plpgsql;`}}class ae extends W{showVersion(){return null}createIndex(e){throw new Error("Redshift not support index!")}dropIndex(e,t){return`DROP INDEX "${t}"`}showIndex(e,t){return`select
    t.relname as table_name,
    i.relname as index_name,
    a.attname as column_name,
    ix.indisprimary "isPrimary",
...
-- since mysql version 5.7, password column need change to authentication_string=PASSWORD("newPassword")`}updateColumn(e,t){var _;const{name:n,type:r,comment:o,nullable:s,defaultValue:l,extra:i,character_set_name:c,collation_name:m}=t,u=s!="YES";return new d(`ALTER TABLE ${e}`).append(`
	CHANGE ${n} ${n} ${r}`).if(c,`CHARACTER SET ${c}`).if(m,`COLLATE ${m}`).if(u,"NOT NULL").if((_=i==null?void 0:i.toLowerCase())==null?void 0:_.includes("auto_increment"),"AUTO_INCREMENT").if(o,`COMMENT '${o}'`).if(A(l)&&!u,"DEFAULT NULL").if(!A(l),`DEFAULT ${l=="CURRENT_TIMESTAMP"?l:`'${V(l)}'`}`).toString()}updateColumnSql(e){const{table:t,columnName:n,newColumnName:r,columnType:o,isNotNull:s,isAutoIncrement:l,comment:i,defaultValue:c,oldRow:m}=e,u=`ALTER TABLE "${t}"`,_=s?"SET NOT NULL":"DROP NOT NULL";return new d(`${u} ALTER COLUMN "${n}" TYPE ${o};`,`
...
AS
SELECT * FROM $2`}procedureTemplate(){throw new Error("Method not implemented.")}triggerTemplate(){throw new Error("Method not implemented.")}functionTemplate(){throw new Error("Method not implemented.")}processList(){throw new Error("Method not implemented.")}variableList(){throw new Error("Method not implemented.")}statusList(){throw new Error("Method not implemented.")}}function se(a){switch(a||(a=E.MYSQL),a){case null:case E.MYSQL:return new G;case E.MARIA_DB:return new Z;case E.MSSQL:return new te;case E.SQLITE:return new oe;case E.REDSHIFT:return new ae;case E.SNOWFLAKE:return new re;case E.PG:return new W;case E.CLICKHOUSE:return new Q;case E.DM:return new J;case E.ORACLE:return new k;case E.ES:return new z;case E.MONGO_DB:return new ee;case E.NEO4J:return new ne}return new X}class M{constructor(e){this.meta=e,this.dialect=se(e.dbType),this.wrappedTable=w(this.meta.table,this.meta.dbType,q.Always)}updateTable(e){return this.dialect.updateTable(e)}columnTypes(){return["INTEGER",{label:"VARCHAR",value:"VARCHAR(255)"},{label:"CHAR",value:"CHAR(50)"},"hr","DATE","TIMESTAMP","hr","BOOLEAN","FLOAT","DOUBLE","hr","TEXT","JSON","BLOB"]}strategys(){return["RESTRICT","CASCADE","SET NULL","SET DEFAULT","NO ACTION"]}newColumn(e){const{name:t,type:n,isNotNull:r}=e,o=r?" NOT NULL":"";return`ALTER TABLE ${this.wrappedTable} ADD COLUMN ${w(t,this.meta.dbType)} ${n}${o}`}updateColumn(e){return this.dialect.updateColumnSql({...e,table:this.meta.table})}dropColumn(e){return`ALTER TABLE ${this.wrappedTable} DROP COLUMN ${w(e,this.meta.dbType)}`}formatIndexType(e){return e.index_type}indexTypes(){return[{label:"INDEX",value:"INDEX"},{label:"UNIQUE",value:"UNIQUE"}]}dropIndex(e){return this.dialect.dropIndex(this.wrappedTable,e)}showChecks(){return this.dialect.showChecks(this.meta.schema,this.meta.table)}showForeignKeys(){return`SELECT
...
WHERE c.table_schema = '${this.meta.schema}' and c.table_name = '${this.meta.table}' ORDER BY ik.CONSTRAINT_NAME;`}newForeignKey(e){const{column:t,foreignKeyName:n,refTable:r,refColumn:o,onUpdate:s,onDelete:l}=e,i=s=="NO ACTION"?"":` ON UPDATE ${s}`,c=l=="NO ACTION"?"":` ON DELETE ${l}`;return new d(`ALTER TABLE "${this.meta.table}"`).if(n,()=>`ADD CONSTRAINT ${n}`).if(!n,"ADD").append(`FOREIGN KEY ("${t}") REFERENCES "${r}"("${o}")${i}${c};`).toString()}dropForignKey(e){return`ALTER TABLE "${this.meta.table}" DROP CONSTRAINT ${e};`}newCheck(e){return`ALTER TABLE ${this.wrappedTable} ADD CHECK (${e});`}dropCheck(e){return`ALTER TABLE ${this.wrappedTable} DROP CONSTRAINT ${e};`}splitColumn(e,t='"'){return e.map(n=>t+n+t).join(",")}}class le extends M{columnTypes(){return["Int32","String","Float32","hr","Date","DateTime","hr","BOOLEAN","UUID",{label:"Array",value:"Array(String)"},"hr","JSON",{label:"Tuple",value:"Tuple(String)"},{label:"Map",value:"Map(String,String)"}]}newColumn(e){const{name:t,type:n,defaultValue:r,comment:o}=e;return new d(`ALTER TABLE ${this.wrappedTable} ADD COLUMN "${t}" ${n}`).if(!A(r),`DEFAULT ${L(r,n)}`).if(o,`COMMENT '${o}'`).toString()}newIndex(e){const{columns:t}=e;return`ALTER TABLE "${this.meta.table}" ADD INDEX ${t.join("")}_${new Date().getTime()}_index expression TYPE type GRANULARITY value AFTER ${this.splitColumn(t)}`}showForeignKeys(){return null}showChecks(){return null}}class ie extends M{newColumn(e){const{name:t,type:n,defaultValue:r,isNotNull:o}=e;return new d(`ALTER TABLE ${w(this.meta.table,this.meta.dbType)} ADD ${w(t,this.meta.dbType)} ${n}`).if(o,"NOT NULL").if(!A(r),`DEFAULT ${L(r,n)}`).toString()}newIndex(e){const{table:t=this.meta.table,columns:n,type:r}=e,o=`${t}_${n.join("_")}_${new Date().getTime()}`;return`CREATE ${r=="UNIQUE"?"UNIQUE":""} INDEX ${o} ON "${t}"(${this.splitColumn(n)});`}showForeignKeys(){return this.dialect.showForeignKeys(this.meta.schema,this.meta.table)}}class ce extends M{indexTypes(){return[{label:"INDEX",value:"INDEX"},{label:"UNIQUE",value:"UNIQUE"},{label:"FULLTEXT",value:"FULLTEXT"}]}columnTypes(){return["INT",{label:"VARCHAR",value:"VARCHAR(255)"},{label:"CHAR",value:"CHAR(50)"},"hr","DATETIME","TIMESTAMP","DATE","hr","BIT","FLOAT","DOUBLE",{label:"DECIMAL",value:"DECIMAL(10,2)"},"BIGINT","hr","TEXT","JSON","BLOB","hr",{label:"ENUM",value:"ENUM('item')"},{label:"SET",value:"SET('item')"}]}newColumn(e){const{name:t,type:n,comment:r,defaultValue:o,isNotNull:s,unsigned:l,zerofill:i}=e;return console.log(e),new d(`ALTER TABLE ${this.wrappedTable} ADD COLUMN \`${t}\` ${n}`).if(l&&l!=0,"UNSIGNED").if(i,"ZEROFILL").if(s,"NOT NULL").if(r,`COMMENT '${r}'`).if(!A(o),`DEFAULT ${L(o,n)}`).toString()}formatIndexType(e){const t=e.index_type.toUpperCase();return t=="BTREE"?e.index_name=="PRIMARY"?"PRIMARY":H(e.isUnique)?"UNIQUE":"KEY":t}newIndex(e){const{type:t="key",columns:n}=e;return`ALTER TABLE \`${this.meta.table}\` ADD ${t} (${this.splitColumn(n,"`")})`}newForeignKey(e){const{column:t,foreignKeyName:n,refTable:r,refColumn:o,onUpdate:s,onDelete:l}=e,i=s=="NO ACTION"?"":` ON UPDATE ${s}`,c=l=="NO ACTION"?"":` ON DELETE ${l}`;return new d(`ALTER TABLE \`${this.meta.table}\``).if(n,()=>`ADD CONSTRAINT ${n}`).if(!n,"ADD").append(`FOREIGN KEY (\`${t}\`) REFERENCES \`${r}\`(\`${o}\`)${i}${c};`).toString()}dropForignKey(e){return`ALTER TABLE \`${this.meta.table}\` DROP FOREIGN KEY ${e};`}}class Ee extends M{newIndex(e){const{table:t=this.meta.table,columns:n,type:r}=e,o=`${t}_${n.join("_")}_${new Date().getTime()}`;return`CREATE ${r=="UNIQUE"?"UNIQUE":""} INDEX ${o} ON "${t}"(${this.splitColumn(n)})`}newColumn(e){const{name:t,type:n,isNotNull:r,defaultValue:o,comment:s}=e;return new d(`ALTER TABLE ${w(this.meta.table,this.meta.dbType)} ADD ${t} ${n}`).if(!A(o),`DEFAULT ${L(o,n)}`).if(r,"NOT NULL").if(s,`;
COMMENT ON COLUMN ${this.wrappedTable}."${t.toUpperCase()}" is '${s}';`).toString()}newForeignKey(e){const{column:t,foreignKeyName:n,refTable:r,refColumn:o,onUpdate:s,onDelete:l}=e,i=s=="NO ACTION"?"":` ON UPDATE ${s}`,c=l=="NO ACTION"?"":` ON DELETE ${l}`,m=n||`${t}_${r}_${o}_fk_${Math.floor(Math.random()*30)}`;return`ALTER TABLE "${this.meta.table}" ADD CONSTRAINT ${m} FOREIGN KEY ("${t}") REFERENCES ${r}(${o})${i}${c};`}showForeignKeys(){var t;let{schema:e}=this.meta;return(e==null?void 0:e.toLowerCase())==((t=this.meta.user)==null?void 0:t.toLowerCase())&&(e=void 0),this.dialect.showForeignKeys(e,this.meta.table)}}class ue extends M{newColumn(e){const{name:t,type:n,comment:r,defaultValue:o,isNotNull:s}=e,l=this.wrappedTable;return new d(`ALTER TABLE ${l} ADD COLUMN "${t}" ${n}`).if(s,"NOT NULL").if(!A(o),`DEFAULT ${L(o,n)}`).append(";").if(r,`COMMENT ON COLUMN ${l}."${t}" is '${r}';`).toString()}columnTypes(){return["INTEGER",{label:"VARCHAR",value:"VARCHAR(255)"},{label:"CHAR",value:"CHAR(50)"},"TIMESTAMP","DATE","BOOLEAN","FLOAT","TEXT","JSON","BLOB"]}indexTypes(){return[{label:"INDEX",value:"INDEX"},{label:"UNIQUE",value:"UNIQUE"},{label:"HASH",value:"HASH"}]}newIndex(e){const{table:t=this.meta.table,columns:n,type:r}=e,o=r=="HASH"?"hash":"btree";return`CREATE ${r=="UNIQUE"?"UNIQUE":""} INDEX ${n.join("_")}_${new Date().getTime()}_index ON "${t}" USING ${o} (${this.splitColumn(n)});`}showForeignKeys(){return`SELECT
  ...
  GROUP BY
      ...
      ccu.table_schema,
      ccu.table_name
  ORDER BY ord_position;`}}class me extends M{newIndex(e){throw new Error("Method not implemented.")}dropForignKey(e){return`ALTER TABLE "${this.meta.table}" DROP CONSTRAINT "${e}";`}showForeignKeys(){return`select
